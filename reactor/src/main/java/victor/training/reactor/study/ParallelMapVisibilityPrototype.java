 ...
 */
public class ParallelMapVisibilityPrototype {
    ...
    public ParallelMapVisibilityPrototype() {
        this.parallelScheduler = Schedulers.newParallel("worker", PARALLELISM);
    }

    /**
     * Demonstrates the pattern where:
     * 1. Items are processed in parallel rails
     * 2. Each rail modifies its own bucket of a shared data structure (BucketedState)
    ...
    private Flux<Result> gatherResultsFromAllBuckets(BucketedState state) {
        return Flux.defer(() -> {
            ...
            for (Map.Entry<String, Set<String>> entry : mergedResults.entrySet()) {
    ...
    static class BucketedState implements AutoCloseable {
        ...
        BucketedState() {
            this.bucketMaps = new HashMap[PARALLELISM];
    ...
    public static void main(String[] args) throws InterruptedException {
        ParallelMapVisibilityPrototype prototype = new ParallelMapVisibilityPrototype();
